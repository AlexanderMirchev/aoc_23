...
defmodule Day3 do
  def solution_a(input) do
    ...
    nums = get_nums(lines)
    symbol_positions = get_symbols(lines)
      
    filtered_nums = nums
      |> Enum.filter(fn {xrange, yrange, _} ->
        symbol_positions  
        |> Enum.any?(fn {x,y} -> x in xrange and y in yrange end )
      end)

     Enum.reduce(filtered_nums, 0, fn {_, _, num}, acc -> acc + num end)
  end
  ...
  def solution_b(input) do
    ...
    |> Enum.map(fn {x,y} -> 
      nums
      |> Enum.filter(fn {xrange, yrange, _} -> x in xrange and y in yrange end)
    end)
    |> Enum.filter(fn arr -> length(arr) == 2 end)
    |> Enum.map(fn [{_,_, a}, {_, _, b}] -> a * b end)
    |> Enum.sum()
  end
...
# IO.puts("Solution #{Day3.solution_a(input)}")
IO.puts("Solution #{Day3.solution_b(input)}")
